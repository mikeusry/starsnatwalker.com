...
function scoreBot(request: Request, body: TrackPayload): { isBot: boolean; reason: string } {
  ...
  // 2) UA tokens — explicit crawlers + headless/automation frameworks. Superset
  //    of the client-side BOT_UA regex (PR #2) so the server enforces it even if
  //    a crawler bypasses the inline script and POSTs here directly.
  if (/bot|crawl|spider|slurp|bingpreview|facebook|mediapartners|headless|phantom|puppeteer|playwright|selenium|python-requests|axios|curl|wget|http-client|scrapy|go-http|node-fetch/i.test(ua)) {
    return { isBot: true, reason: 'ua:token' };
  }
  if (!ua) return { isBot: true, reason: 'ua:empty' };

  // 2b) Own-hub referral loop (ported from PR #2): a page_view that refer-chained
  //     off one of OUR hub pages is the crawler walking /recruiting → each featured
  //     player. Real coaches arrive from email (?c=&s=), search, or direct — never
  //     by walking our internal hub. The browser sends document.referrer in the
  //     payload; we enforce it here so it can't be bypassed.
  if (/starsnatwalker\.com\/(recruiting|pitchers|catchers|roster)\b/i.test(referrer)) {
